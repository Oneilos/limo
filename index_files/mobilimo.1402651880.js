...
Mobi.filtreLoaderMission = 'MIS_DATE_DEBUT >="'+Jaf.date2mysql((new Date()).getTime()-3*24*3600000)+'" AND MIS_DATE_DEBUT <="'+Jaf.date2mysql((new Date()).getTime()+7*24*3600000)+'"';
...
Mobi.listeConcepts = {
    ...
    C_Com_TypeService : '' ,
    C_Com_Commande             : 'COM_ID     in (select distinct MIS_COM_ID from nf_gen_mission where ' + Mobi.filtreLoaderMission + ')',
    C_Com_FraisMission         : 'FMI_MIS_ID in (select MIS_ID              from nf_gen_mission where ' + Mobi.filtreLoaderMission + ')',
    C_Gen_StatutMission : '' ,
    C_Gen_Chauffeur : '' ,
    C_Gen_Client : '' ,
    C_Gen_Contact : '' ,
    C_Gen_Mission              : Mobi.filtreLoaderMission ,
    C_Gen_Partenaire : '' ,
    C_Gen_Voiture : '' ,
    C_Gen_TypeVehicule : '' ,
    C_Gen_Civilite : '' ,
...
Mobi.init = function() {
    switch ( Mobi.step ) {
        ...
        case 1 : //contruction des tables
            ...
            for(var i in Mobi.listeConcepts) {
                Jaf.cm.getConcept(i).installSql(Mobi.listeConcepts[i]); 
            }
        ...
        case 4 : //chargement des datas des tables installées
                if ( Mobi.newversion ) {
...
Mobi.initConnexion = function() {
    ...
    if ( login == null || login.length==0 ) {
        ...
        var trans = $.ajax({  
            url  : 'http://dev.limovtc.fr/bop2/index.php', 
        ...
        trans.fail(function() {
            ...
            $('#init .infos').append('<div>la connexion ne peut être établie</div>');
            Mobi.step=1;
            Mobi.init();
        });
    }
...
Mobi.bindSqlInstalled = function (concept,type,nb) {
    if ( type ) { 
        ...
        Jaf.log(concept+' : '+type+' ==> '+Mobi.nbProcessus.loaded+'/'+Mobi.nbProcessus.loading+', '+Mobi.nbProcessus.saved+'/'+Mobi.nbProcessus.saving);
    ...
    switch ( Mobi.step ) {
        case 1 :
            ...
            coef = ( Mobi.nbProcessus.created + Mobi.nbProcessus.loaded )  / Mobi.nbProcessus.init / 2;
            $('#init .progression').css('width',Math.round( 10 + coef * 50 )+'%');

        break;
        case 2 :
            if ( Mobi.nbProcessus.saved  == Mobi.nbProcessus.saving && 
                 Mobi.nbProcessus.loaded == Mobi.nbProcessus.loading && 
                 Mobi.nbProcessus.init   == Mobi.nbProcessus.loaded )  {
                Mobi.step=3;
            }
            coef = Mobi.nbProcessus.saved / Mobi.nbProcessus.saving;
            $('#init .progression').css('width',Math.round( 60 + coef * 20 )+'%');
        break;
        case 3 :
...
Mobi.initAffichage = function() {
	...
	monform.find('input[name=CLI_SOCIETE]').autocomplete({
		 ...
		 select  : function( event, ui ) {
			...
			monform.find('input[name=CLI_ID]').val(ui.item.value);
            Mobi.creationDossierClientContact(monform,ui.item.value); 
    ...
    $('#menu .suivi').click(function () {
        ...
        if ( chu_id>0) {
           window.open('/bop3/C_Gen_Chauffeur/connexion-en-tant-que/?CHU_ID='+chu_id); 
        } else {
...
Mobi.fonctionsCel.getDecalage = function(lie_id) {
	if ( lie_id > 0 ) {
		...
		if ( ville.VIL_DECALAGE_HORAIRE != 0) {
			return ville.VIL_DECALAGE_HORAIRE;
		} else {
			var pays = Jaf.cm.getConcept('C_Geo_Pays').getRow(ville.VIL_PAY_ID);
			if ( pays.PAY_DECALAGE_HORAIRE != 0) {
				return pays.PAY_DECALAGE_HORAIRE;
			}
			return 0;
		}
	}
...
Mobi.depannage = function() {
    ...
    zoneMessage.find('.razBt').click(function() {
        zoneMessage.find('textarea[name=message]').html('');
    });
    return false;
}
...
Mobi.debuggage = function () {
    ...
    var trans = $.ajax({
        url   : 'http://dev.limovtc.fr/mobilimo/debuggage',
        type  : 'POST',
        data  : datas,
        cache : false
    });
...
Mobi.creationDossierClientContact = function(monform,cli_id) {
    ...
    if ( cli_id > 0 ) {
		var client   = Jaf.cm.getConcept('C_Gen_Client').getRow(cli_id);
		var contacts = Jaf.cm.getConcept('C_Gen_Contact').getSelect().join('C_Gen_Civilite','COT_CIV_ID','CIV_ID').fetchAll({COT_CLI_ID:cli_id});
		var liste    = '<option value="0">...</option>';
		var cot_id   = 0;
        for(var i in contacts) {
			var civ = contacts[i].COT_CIV_ID > 0 ? Jaf.translate(contacts[i].CIV_LIBELLE_COURT)+' ' : ''; 
			if ( contacts[i].COT_DROIT_VALIDATION == 1 || cot_id==0 ) cot_id = contacts[i].COT_ID;
            liste += '<option value="'+contacts[i].COT_ID+'">'+civ+contacts[i].COT_PRENOM+' '+contacts[i].COT_NOM.toUpperCase()+'</option>';
		}
...
Mobi.ouvreLieu = function (lie_id,type) {
    ...
    $('#fiche').find('input,select,textarea').each(function() {
        ...
        concept.setValue( mis_id , $(this).attr('name') , valeur );
    });
    concept.save();
