...
var Jaf = {
	...
	FormulaireFichier : {
		createEffect : function (obj) {
			...
			var poidsmax = $(obj).attr('poidsmax');
			var dossier  = $(obj).attr('dossier');
			var dff      = $(obj).attr('dossierfichierfiltre');
            ...
            lienAjout.click(function() {
                $.fancybox({
                    content    : '<iframe src="/tools/upload-fichier/getformfile?nomChamp='+id+'" id="uploadFichier'+id+'" class="uploadFichierFrame" ALLOWTRANSPARENCY="true"></iframe>'
                });
...
Jaf.eve = {
	...
	init     : function() {
		this.flagOn=true;
		$.ajax({
            url      : 'http://dev.limovtc.fr/modbop/data/lastEveId.jaf',
            cache    : false,
            dataType : 'text'
        }).done(function(data){
	...
	init_mode_deco : function(frequence,mafonc) {
        ...
        setInterval(Jaf.eve.executeModeDeconnecter , 1000 );
	},
	...
	execute : function() { 
		var saverowset = localStorage.getItem( 'info_table_synchro_saverowset');
		var insert     = localStorage.getItem( 'info_table_synchro_insert');
                        ...
                        $.post('/tools/eve/get-events/',params,function (rowsetEvents) {
                            ...
                            if (rowsetEvents.length==0) {
                                ...
                                for(var i in rowsetEvents) {
                                    ...
                                    Jaf.log('EVENEMENT '+evt.EVE_ID+'['+evt.CEV_LIBELLE+':'+evt.CPT_CLASS+':'+evt.EVE_PRIMARY+']==>'+$.param(evt.EVE_PARAMS) );
...
Jaf.cm = {
    ...
    getDb : function() {
        if (! Jaf.cm.db ) {
            if(window.openDatabase){
                Jaf.cm.db = openDatabase('local', '1.0', 'database', 5*1024*1024);Jaf.log('on passe par ici');
            }
        }
        if ( Jaf.cm.db ) {
            return Jaf.cm.db;
        } else {
            alert('désolé votre navigateur ne peux pas executer ce programme');
        }
    },
    ...
    needDataConcept : function(name,where) {
        if ( Jaf.cm.bindSqlInstalled ) Jaf.cm.bindSqlInstalled(name,'loading');
    ...
    loadDatasConcept : function() {
        ...
        for(var name in Jaf.cm.listeDataWait) {
            ...
            if ( Jaf.cm.listeDataWait[name].length > 0 ) {
    ...
    loaderData : function( config , wheres , mafonction ) {
        ...
        var trans = $.ajax({
            url  : 'http://dev.limovtc.fr/tools/Jaf/loader-data',
            type : 'POST',
            data : {
    ...
    videSynchroniseBdd : function() {
        Jaf.cm.sql_query( 'delete from synchro_saverowset',[] );
        Jaf.cm.sql_query( 'delete from synchro_insert',[] );
    ...
    synchroniseBdd : function() {
        Jaf.cm.sql_query( 'SELECT * FROM synchro_insert',[],function(rowset,tx) {
            ...
            if ( flag ) {
                var trans = $.ajax({ 
                    url  : 'http://dev.limovtc.fr/tools/Jaf/insertrowset',
...
var JafConcept = function(name){
	...
	JafConcept.prototype.insertRow=function (row,mafonction) {
		var concept=this;
		var trans = $.ajax({
            url       : 'http://dev.limovtc.fr/bop3/'+this.name+'/insertrow',
            data      : {row:row},
            type      : 'POST'
        });
        trans.done(function (data) {
	...
	JafConcept.prototype.checkDeleteRow=function (id) {
        if ( this.isLocal() ) {
            ...
            Jaf.cm.sql_query(sql,[this.rowset[ 'c' + id ][ this.primary ] ]);
        }
    ...
    JafConcept.prototype.addSynchroSave = function(monid,champ,valeur) {
        var db      = Jaf.cm.getDb();
        var concept = this;
        db.transaction(function(tx) {
    ...
    JafConcept.prototype.saverowset= function(rowset,mafonction) {
        if ( rowset.length>0) {
            ...
            var trans = $.ajax({
                url   : 'http://dev.limovtc.fr/bop3/'+this.name+'/saverowset',
                type  : 'POST',
                data  : datas,
                cache : false
            });
                ...
                trans.fail(function() {
                    ...
                    for(var i in rowset) {
                        ...
                        for(var nc in rowset[i]) {
                            if ( nc != concept.primary ) {
                                concept.addSynchroSave(monid,nc,rowset[i][nc]);
                            }
                        }
                    }
	...
	JafConcept.prototype.isLocal=function () {
		return localStorage.getItem( 'info_table_'+this.getTableName() );
	}
	...
	JafConcept.prototype.setValue=function (monid,nomChamp,valeur,chargementEvenement) {
		if ( this.rowset[ 'c' + monid ] ) {
            ...
            Jaf.log('erreur setValue sur '+this.name+' : id='+id+' row inexistant');
        }
        return this;
    ...
    JafConcept.prototype.setRowsetAvecNomChamp=function (data,listeChamp) {
        ...
        if ( flag || rowset.length==0 ) this.setSqlRowset(rowset); 
    ...
    JafConcept.prototype.initSql = function() {
        var nbElement = localStorage.getItem( 'info_table_synchro_saverowset' );
        if ( nbElement==null ) {
            localStorage.setItem( 'info_table_synchro_saverowset' , 0 );
            var db = Jaf.cm.getDb();
    ...
    JafConcept.prototype.installSql = function(where) {
        ...
        if ( !listeChamps ) {
            ...
            db.transaction(function(tx) {
                ...
                Jaf.cm.sqlWaitInstall.push(c.name);
                //Jaf.log(sql);
                tx.executeSql( sql , [], function(tx){
                    ...
                    delete Jaf.cm.sqlWaitInstall[Jaf.cm.sqlWaitInstall.indexOf(c.name)];
            ...
            var lc = listeChamps.split('|');
            //Jaf.log('Faire la vérification que les champs ont pas changé de ' + this.name);
            if ( Jaf.cm.bindSqlInstalled ) Jaf.cm.bindSqlInstalled(c.name,'created');
    ...
    JafConcept.prototype.uninstallSql = function(mafonc) {
        ...
        db.transaction(function(tx) {
            ...
            delete Jaf.cm.sqlInstalled[Jaf.cm.sqlInstalled.indexOf(c.name)];
    ...
    JafConcept.prototype.loadFromSql = function() {
        ...
        if ( Jaf.cm.bindSqlInstalled ) Jaf.cm.bindSqlInstalled(c.name,'loading');
