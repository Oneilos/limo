...
Jaf.eve = {
	...
	init     : function() {
		this.flagOn=true;
		$.ajax({
            url      : Jaf.urlSite + 'modbop/data/lastEveId.jaf',
            cache    : false,
            dataType : 'text'
        }).done(function(data){
...
Jaf.tm = {
	...
	require     : function (filename) {
		if ( !Jaf.tm.ficTml[filename] ) {
			Jaf.tm.TmlWaiting++;
			$.ajax({
				url      : Jaf.urlSite + Jaf.tm.dossier +'/' + filename,
                dataType : 'text'
	...
	require_new: function (filename) {
		if ( !Jaf.tm.ficTml[filename] ) {
			Jaf.tm.TmlWaiting++;
			$.ajax({
				url      : Jaf.urlSite + 'jsonp.php?fichier=' + Jaf.tm.dossier + '/' + filename,
                dataType : 'jsonp'
			}).done(function (tmlContent) {
				var regex = /([a-zA-Z0-9_-]+).([0-9]+).tml/gi
                var res = regex.exec(filename);
                filename = res[1]+'.tml';
				Jaf.tm.ficTml[filename] = tmlContent.tml;
                Jaf.tm.multiCompile(filename,tmlContent.tml);
				Jaf.tm.TmlWaiting--;
				Jaf.tm.checkOnLoad();
			});
		}
