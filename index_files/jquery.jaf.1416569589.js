...
var Jaf = {
    ...
    setCkId      : function(data) {
        Jaf.setEnv('ck_id',data.ck_id);
    },
	...
	Bop3 : {
		...
		post : function ( concept , action , params ) {
			$.post(Jaf.urlSite + Jaf.cm.urlDb + concept + '/' + action,
		    		params, 
		    		function(data) {eval(data);});
		},
		getrow : function ( concept , id , mafonction ) {
			$.get(Jaf.urlSite + Jaf.cm.urlDb + concept + '/getrow',
		    		{ID:id}, 
		    		function(data) {
    ...
    maps : {
        loadScript : function() {
            if ( ! Jaf.maps.scriptLoaded ) {
                $('body').append( '<script type="text/javascript" src="https://maps.googleapis.com/maps/api/js?sensor=false&language=fr&callback=initializeGoogleApi"></script>');
                Jaf.maps.scriptLoaded = true;
            }
        },
        initEffect : function() {
	...
	FormulaireFichier : {
		createEffect : function (obj) {
            ...
            if (obj.length>0) {
                ...
                obj.after(lienAjout);
                $.post( Jaf.urlSite + 'tools/upload-fichier/load',params,function(data) {
...
Jaf.eve = {
	...
	init     : function() {
		this.flagOn=true;
		$.ajax({
            url      : Jaf.urlSite + 'jsonp.php?fichier=modbop/data/lastEveId.jaf',
            cache    : false,
            dataType : 'jsonp'
        }).done(function(data){
            Jaf.cm.doneTransaction();
            Jaf.eve.lastEveId=data.content;
	...
	addEvent : function(params)  {
		...
		$.post( Jaf.urlSite + 'tools/eve/add-event',params);
	},
    ...
    setEvent : function (rowsetEvents) {
        ...
        if ( Jaf.eve.mafonc ) Jaf.eve.mafonc( { type : 'loadNewEvent', resultat : 'ok' } );
    },
    setEventJsonP     : function(data) {
        Jaf.eve.setEvent(data.rowsetEvents);
        Jaf.eve.lastEveId = data.lastEveId;
    },
    loadNewEvent : function(mafonc) {
        ...
        var pAjax = {
            url           : Jaf.urlSite + 'tools/eve/get-events/',
            type          : 'POST',
            cache         : false,
            dataType      : 'jsonp',
            data          : params,
            jsonpCallback : 'Jaf.eve.setEventJsonP'
        }
        Jaf.eve.mafonc = mafonc;
        
        $.ajax( pAjax ).fail( function() {
            Jaf.cm.failTransaction();
            if ( mafonc ) mafonc( { type : 'loadNewEvent', resultat : 'ko' } );
        });
	},
	execute : function() { 
		var saverowset = 1*localStorage.getItem( Jaf.cm.preFixe+'info_table_synchro_saverowset');
		var insert     = 1*localStorage.getItem( Jaf.cm.preFixe+'info_table_synchro_insert');
        ...
        if ( !Jaf.eve.flagExecute ) {
            ...
            $.ajax({
                ...
                Jaf.cm.doneTransaction();
                if ( nb == 0 ) {
                    if ( data && Jaf.eve.lastEveId!=data.content) {
                        $.ajax({
                            url      : Jaf.urlSite + 'tools/eve/get-events/',
                            cache    : false,
                            data     : params,
...
Jaf.cm = {
    ...
    modeEmbarquer         : false,
    initSql               : function() {
        var nbElement = localStorage.getItem( Jaf.cm.preFixe+'info_table_synchro_saverowset' );
        if ( nbElement==null ) {
            localStorage.setItem( Jaf.cm.preFixe+'info_table_synchro_saverowset' , 0 );
            var db = Jaf.cm.getDb();
        ...
        var nbElement = localStorage.getItem( Jaf.cm.preFixe+'info_table_synchro_insert' );
        if ( nbElement==null ) {
            localStorage.setItem( Jaf.cm.preFixe+'info_table_synchro_insert' , 0 );
            var db = Jaf.cm.getDb();
            db.transaction(function(tx) { 
    ...
    loaderData : function( config , wheres , mafonction ) {
        ...
        var pAjax = {
            url  : Jaf.urlSite + Jaf.cm.urlLoaderData,
            type : 'POST',
            dataType : Jaf.cm.modeEmbarquer ? 'jsonp' : 'text',
            data : data
        };
        
        if ( Jaf.cm.modeEmbarquer ) {
            pAjax.jsonpCallback = 'Jaf.cm.loadData';
        }
        
        var trans = $.ajax(pAjax);
    ...
    videSynchroniseBdd : function() {
        ...
        localStorage.removeItem( Jaf.cm.preFixe+'info_table_synchro_saverowset');
        localStorage.removeItem( Jaf.cm.preFixe+'info_table_synchro_insert' );
    ...
    deleteSyi : function(syi_id) {
        Jaf.cm.sql_query( 'DELETE FROM synchro_insert WHERE SYI_ID = ? ' , [ syi_id ] ,function() {
            var nbElement = localStorage.getItem( Jaf.cm.preFixe+'info_table_synchro_insert');
            localStorage.setItem( Jaf.cm.preFixe+'info_table_synchro_insert' , nbElement - 1 );
        }); 
    },
        ...
        Jaf.cm.sql_query( 'SELECT * FROM synchro_saverowset',[],function(rowset,tx) {
            ...
            for(var i in rowset) {
                ...
                tx.executeSql( 'delete from synchro_saverowset where SYN_ID=?' , [row.SYN_ID] ,function() {
                    var nbElement = localStorage.getItem( Jaf.cm.preFixe+'info_table_synchro_saverowset');
                    localStorage.setItem( Jaf.cm.preFixe+'info_table_synchro_saverowset' , nbElement - 1 );
                });
            }
    ...
    synchroniseBdd : function() {
        Jaf.cm.sql_query( 'SELECT * FROM synchro_insert',[],function(rowset,tx) {
            ...
            if ( flag ) {
                var trans = $.ajax({ 
                    url  : Jaf.urlSite + 'tools/Jaf/insertrowset',
...
var JafConcept = function(name){
	...
	JafConcept.prototype.insertRow=function (row,mafonction) {
		var concept=this;
		var trans = $.ajax({
            url       : Jaf.urlSite + Jaf.cm.urlDb + this.name + '/insertrow',
            data      : {row:row},
            type      : 'POST'
        });
        trans.done(function (data) {
	...
	JafConcept.prototype.deleteRow=function ( id,mafonction) {
		...
		var concept=this;
        $.post(Jaf.urlSite + Jaf.cm.urlDb + this.name + '/deleterow',{ id : id } ,function () {
			if (mafonction) 
				mafonction(id);
		}); 
		return this;
    ...
    JafConcept.prototype.addSynchroSave = function(monid,champ,valeur,madate) {
        ...
        db.transaction(function(tx) {
            ...
            tx.executeSql( sql , [ 
                ...
                var nbElement = 1*localStorage.getItem( Jaf.cm.preFixe+'info_table_synchro_saverowset' );
                localStorage.setItem( Jaf.cm.preFixe+'info_table_synchro_saverowset' , nbElement + 1 );
            });
        });
    }
    ...
    JafConcept.prototype.addSynchroInsert = function(row) {
        ...
        db.transaction(function(tx) {
            ...
            tx.executeSql( sql , [ 
                ...
                var nbElement = 1*localStorage.getItem( Jaf.cm.preFixe+'info_table_synchro_insert' );
                localStorage.setItem( Jaf.cm.preFixe+'info_table_synchro_insert' , nbElement + 1 );
            });
        });
    }
    ...
    JafConcept.prototype.saverowset= function(rowset,mafonction) {
        if ( rowset.length>0) {
            ...
            var trans = $.ajax({
                url   : Jaf.urlSite + Jaf.cm.urlDb + this.name + '/saverowset',
                type  : 'POST',
                data  : datas,
                cache : false
            });
	...
	JafConcept.prototype.isLocal=function () {
        return Jaf.eve.mode_deco && localStorage.getItem( Jaf.cm.preFixe+'info_table_'+this.getTableName() );
	}
	...
	JafConcept.prototype.fetchAll=function (params,mafonction) {
		...
		$.post( Jaf.urlSite + Jaf.cm.urlDb + this.name + '/getlistejson',params,function (data) {
			eval(data);
    ...
    JafConcept.prototype.installSql = function(where) {
        ...
        var listeChamps = localStorage.getItem( Jaf.cm.preFixe+'info_table_'+c.name );
        ...
        if ( !listeChamps ) {
            ...
            db.transaction(function(tx) {
                ...
                tx.executeSql( sql , [], function(tx){
                    ...
                    localStorage.setItem( Jaf.cm.preFixe+'info_table_'+c.name ,  listeChamps.join('|') );
    ...
    JafConcept.prototype.uninstallSql = function(mafonc) {
        ...
        db.transaction(function(tx) {
            ...
            localStorage.removeItem( Jaf.cm.preFixe+'info_table_'+c.name );
            tx.executeSql( sql , [], function(tx){
        ...
        switch ( this.step ) {
            ...
            case 3 :
                ...
                if ( this.nbElementSynchro ) {
                   var restant = localStorage.getItem( Jaf.cm.preFixe+'info_table_synchro_insert') + localStorage.getItem( Jaf.cm.preFixe+'info_table_synchro_saverowset');
                } else {
                    this.nbElementSynchro = localStorage.getItem( Jaf.cm.preFixe+'info_table_synchro_insert') + localStorage.getItem( Jaf.cm.preFixe+'info_table_synchro_saverowset');
